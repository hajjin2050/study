...
from tensorflow.keras.layers import Dense,SimpleRNN # RNN에서 가장 대표적인 모델, RNN은 Y가 없는 데이터

model = Sequential()
# model.add(LSTM(10, activation='relu', input_shape=(3,1)))# LSTM(10 ->(앞에있는 숫자는 아웃풋)/(4,3,1)에서 가장앞에서 쳐내기(=행무시)
model.add(SimpleRNN(20, activation='relu', input_length=3 ,input_dim=1))#input_length=3 ,input_dim=1  =  input_shape=(3,1)
...
'''
SimpleRNN 정리
activation 의 default값은 "linear"

parameter아웃값 원리 :  (outputnode(파라미터 아웃값) + input_dim(디멘션 값) + 1(바이어스))*outputnode(파라미터 아웃값)
