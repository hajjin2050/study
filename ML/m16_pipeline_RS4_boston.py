...
parameters = [
    ...
    {'randomforestregressor__max_depth' : [6, 8]},
...
for i in scale:
    pipe = make_pipeline(i, RandomForestRegressor()) #[]를 넣어서 오류가 났었음
    for j in search:
        model = j(pipe, parameters, cv=5)
        model.fit(x_train, y_train)
        print(f'score{i}_{j.__name__}', model.score(x_test, y_test))

# scoreMinMaxScaler()_RandomizedSearchCV 0.9431560402684565
# scoreMinMaxScaler()_GridSearchCV 0.9693579352036372
# scoreStandardScaler()_RandomizedSearchCV 0.9530607937958092
# scoreStandardScaler()_GridSearchCV 0.9502165434356479